...
class Map {
  constructor(_svg, _view, _setting, _rect, _offset) {
    ...
    _d3.addT(this.gs.date, "t_label mod-min", "0,75", this.setting.label)
  ...
  setData(_datas) {
    ...
    this.dateView = new SVG_DateView(_d3.addG(this.gs.date, "mod-date", "0,30"))
    this.dateView.update(this.setting.startDate)
    if (this.setting.useClock) {
      this.clockView = new SVG_ClockView(_d3.addG(this.svg, "mod-clock", "440,90"), 35)
    ...
    datas.hi2 = _.sortBy(datas.hi2, [function(o) {
      return o.mag;
    }]);
    for (var i = 0; i < datas.main.length; i++) {
  ...
  drawMap() {
    ...
    this.gs.map_bg2.selectAll("circle").data(datas.hi)
      ...
      .attr("r", 0)
      .transition().duration(100).delay((d, i) => this.getDelay(d.date))
      .attr("r", d => getSize(d.intensity, 2) / 2)
